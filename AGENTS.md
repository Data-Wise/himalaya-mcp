...
> **Agents:** the full suite runs in **~24s on the host, ~53s in CI** (v2.1.2+). If you remember
> it as a ~6-minute run, that was issue #139: `doctor --pre-release` and the suite spawned each
> other with no recursion guard, so the suite spent ~93% of its wall time running nested copies
> of itself and leaked 40+ orphaned vitest processes per run. Fixed — `doctor` now skips its
> test-suite check when `process.env.VITEST` is set.
>
