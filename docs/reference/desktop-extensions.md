...
```bash
himalaya-mcp install-ext himalaya-mcp-v1.6.2.mcpb   # Install
...
src/index.ts (16 source files)
    ...
    │
    └── mcpb pack mcpb/         →  himalaya-mcp-v1.6.2.mcpb (151 KB)
```
...
```bash
mcpb sign himalaya-mcp-v1.6.2.mcpb --key private.pem
mcpb verify himalaya-mcp-v1.6.2.mcpb --key public.pem
```
