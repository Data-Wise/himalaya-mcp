...
```bash
npx @anthropic-ai/mcpb pack mcpb/ himalaya-mcp-v2.1.1.mcpb
```
...
```bash
himalaya-mcp install-ext himalaya-mcp-v2.1.1.mcpb   # Install
...
src/index.ts (16 source files)
    ...
    │
    └── mcpb pack mcpb/         →  himalaya-mcp-v2.1.1.mcpb (253 KB)
```
...
```bash
mcpb sign himalaya-mcp-v2.1.1.mcpb --key private.pem
mcpb verify himalaya-mcp-v2.1.1.mcpb --key public.pem
```
